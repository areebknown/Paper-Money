...
import { Plus, Clock, TrendingUp, CheckCircle, AlertCircle, RefreshCw, Trash2 } from 'lucide-react';
...
export default function AuctionsListPage() {
    ...
    const handleDelete = async (e: React.MouseEvent, id: string, name: string) => {
        ...
        if (!confirm(`Are you sure you want to PERMANENTLY delete the auction "${name}"? This cannot be undone.`)) {
            return;
        }
                ...
                <div className="space-y-4">
                    {auctions.length === 0 ? (
                        ...
                        auctions.map((auction) => (
                            <div key={auction.id} className="group bg-gray-900/40 border border-gray-800/60 hover:border-red-500/30 rounded-xl p-5 transition-all hover:bg-gray-900/60 flex items-start justify-between">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {auction.status === 'LIVE' && (
                                        <button
                                            ...
                                            onClick={async (e) => {
                                                ...
                                                if (!confirm(`Force end "${auction.name}" immediately?`)) return;
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => handleDelete(e, auction.id, auction.name)}
                                        className="p-2 ml-2 text-red-500 bg-red-500/10 hover:bg-red-500 text-sm font-bold hover:text-white rounded-lg transition border border-red-500/20"
