...
export default function BottomNav() {
    ...
    return (
        <nav className=\"fixed bottom-0 left-0 right-0 z-sticky bg-[var(--color-bg-card)] border-t border-[var(--color-border)] pb-safe\">
        ...
        NAV_ITEMS.map((item) => {
            ...
            return (
                <Link
                  ...
                  ${isActive ? 'text-[var(--color-accent-primary)]' : 'text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)]'}
                `}
                >
                    {item.isCenter ? (
